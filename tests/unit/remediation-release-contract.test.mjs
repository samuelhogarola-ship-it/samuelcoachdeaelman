...
test("production docs defer hardened contact without blocking the current form", async () => {
  const [contactSource, contactDocs, runbook, next] = await Promise.all([
  ...
  assert.match(runbook, /Do not deploy `contact` in this rollout/i);
  assert.match(next, /Turnstile \/ contacto endurecido.*no bloqueante/is);
  assert.match(next, /fallback de email actuales/i);
});
